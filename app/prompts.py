def get_email_prompt(email_type, tone, length, keywords=None, cta=None):
    return f"""
        ...
        {f"Include a call-to-action: {cta}." if cta else ""}
...
def get_social_media_prompt(platform, style, tone, length, mentions=None, keywords=None):
    return f"""
    Generate a {length}-character caption for {platform} in a {style} style and {tone} tone.
